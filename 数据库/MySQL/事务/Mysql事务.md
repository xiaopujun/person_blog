...
如果一个事务在执行过程中发生错误或被用户手动回滚，MySQL会使用undo log中的信息来将数据恢复到修改前的状态。这样，即使事务中的某些操作已经执行，但由于事务具有原子性，所以这些操作也会被撤销，从而保证了事务的原子性。
...
> 基于上面的描述的MySQL持久性机制。如果在redolog还未写入磁盘时，发生当季或其他意外。这个事务会被怎么处理？
